-- Add binary file storage capability to resume_files_email table
-- This migration allows storing PDF files directly in PostgreSQL

BEGIN;

-- Add binary data column for file storage
ALTER TABLE resume_files_email 
ADD COLUMN IF NOT EXISTS file_data BYTEA,
ADD COLUMN IF NOT EXISTS storage_type VARCHAR(20) DEFAULT 'database' CHECK (storage_type IN ('database', 'filesystem'));

-- Add index for better performance on file operations
CREATE INDEX IF NOT EXISTS idx_resume_files_email_storage_type ON resume_files_email(email, storage_type);
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'check_file_storage'
    ) THEN
        ALTER TABLE resume_files_email 
